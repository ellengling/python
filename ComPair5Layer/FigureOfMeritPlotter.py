	...
	for angleSelection in angleSelections:
		...
		if doplot:
			if collapse == False:
				...
				AMEGO = numpy.loadtxt(fname='G_AngRes_0.csv', delimiter=',')
				LAT = numpy.loadtxt(fname='LAT_AngRes.csv', delimiter=',')
				COMPTEL = numpy.loadtxt(fname='COMPTEL_AngRes.csv', delimiter=',')

				plot.scatter(Energy[i],st[i],color='black')
				plot.plot(Energy[i], st[i], color='black', alpha=0.5, label='Compton ComPair', lw=2)

				plot.scatter(AMEGO[0:7,0],AMEGO[0:7,1],color='darkgreen')
				plot.plot(AMEGO[0:7,0],AMEGO[0:7,1],color='darkgreen', alpha=0.5, label='Compton AMEGO',lw=2)

				plot.scatter(LAT[0:,0],LAT[0:,1],color='darkblue')
				plot.plot(LAT[0:,0],LAT[0:,1],color='darkblue', alpha=0.5, label='Fermi-LAT',lw=2)

				plot.scatter(COMPTEL[0:,0],COMPTEL[0:,1],color='darkorange')
				plot.plot(COMPTEL[0:,0],COMPTEL[0:,1],color='darkorange', alpha=0.5, label='COMPTEL',lw=2)

				#plot.scatter(Energy[j],sut,color='blue')
				#plot.plot(Energy[j], sut, color='blue', alpha=0.5, label='Compton (untracked)')

				plot.scatter(Energy[k],sp[k],color='black')
				plot.plot(Energy[k],sp[k], color='black', alpha=0.5, ls='--', label='Pair ComPair', lw=2)

				plot.scatter(AMEGO[8:,0],AMEGO[8:,1],color='darkgreen')
				plot.plot(AMEGO[8:,0],AMEGO[8:,1],color='darkgreen', ls='--', alpha=0.5, label='Pair AMEGO',lw=2)
...
def plotEnergyResolution(data, angleSelections=[1,0.9,0.8,0.7,0.6,0.5], xlog=True, ylog=False, save=False, collapse=False):
	...
	for angleSelection in angleSelections:
		...
		if collapse==False:
			...
			AMEGO = numpy.loadtxt(fname='G_EnergyRes_0.csv', delimiter=',')
			LAT = numpy.loadtxt(fname='Lat_EnergyRes.csv', delimiter=',')

			plot.scatter(Energy[i],st,color='black')
			plot.plot(Energy[i], st, color='black', alpha=0.5, lw=2, label='Compton ComPair')

			plot.scatter(AMEGO[0:6,0],AMEGO[0:6,1],color='darkgreen')
			plot.plot(AMEGO[0:6,0],AMEGO[0:6,1],color='darkgreen', alpha=0.5, label='Compton AMEGO',lw=2)
			...
			plot.scatter(Energy[k],sp,color='black')
			plot.plot(Energy[k],sp, color='black', ls='--',alpha=0.5, lw=2, label='Pair ComPair')

			plot.scatter(AMEGO[6:,0],AMEGO[6:,1],color='darkgreen')
			plot.plot(AMEGO[6:,0],AMEGO[6:,1],color='darkgreen',ls='--', alpha=0.5, label='Pair AMEGO',lw=2)
		...
		if plotNumber == len(angleSelections):
			...
			plot.legend(numpoints=1, scatterpoints=1, fontsize=10, frameon=True, loc='upper right')
...
def plotEffectiveArea(data, angleSelections=[1,0.9,0.8,0.7,0.6,0.5], ideal=False, xlog=True, ylog=False, \
	...
	for angleSelection in angleSelections:
		...
		if collapse == False:
			...
			AMEGO = numpy.loadtxt(fname='G_EffArea_0.csv', delimiter=',')
			LAT_Total = numpy.loadtxt(fname='LAT_TotalEffectiveArea.csv', delimiter=',')
			LAT_Front = numpy.loadtxt(fname='LAT_FrontEffectiveArea.csv', delimiter=',')
			COMPTEL = numpy.loadtxt(fname='COMPTEL_EffectiveArea.csv', delimiter=',')

			plot.scatter(Energy, EffectiveArea_Tracked, color='black')
			plot.plot(Energy, EffectiveArea_Tracked, color='black', alpha=0.5, lw=2, label='Compton ComPair')

			plot.scatter(AMEGO[0:8,0],AMEGO[0:8,1],color='darkgreen')
			plot.plot(AMEGO[0:8,0],AMEGO[0:8,1],color='darkgreen', alpha=0.5, label='Compton AMEGO',lw=2)

			plot.scatter(LAT_Total[0:,0],LAT_Total[0:,1],color='darkblue')
			plot.plot(LAT_Total[0:,0],LAT_Total[0:,1],color='darkblue', alpha=0.5, label='Fermi-LAT Total',lw=2)

			plot.scatter(LAT_Front[0:,0],LAT_Front[0:,1],color='darkred')
			plot.plot(LAT_Front[0:,0],LAT_Front[0:,1],color='darkred', alpha=0.5, label='Fermi-LAT Front',lw=2)
			...
			plot.scatter(Energy, EffectiveArea_Pair, color='black')
			plot.plot(Energy, EffectiveArea_Pair, color='black',ls='--', alpha=0.5, lw=2, label='Pair ComPair')
		...
		if plotNumber == len(angleSelections):
			...
			plot.legend(numpoints=1, scatterpoints=1, fontsize=13, frameon=True, loc='lower right')
...
def plotAllSourceSensitivities(data, angleSelection=0.8, plotIdeal=False, xlog=True, ylog=True, \
	...
	a=ascii.read("../../python/data/digitized_alex_sensitivities.dat",names=['eng','sensit'])
	l=ascii.read("../../python/data/differential_flux_sensitivity_p8r2_source_v6_all_10yr_zmax100_n10.0_e1.50_ts25_000_090.txt",names=['emin','emax','e2diff','tmp'])
...
#	comptel_sensitivity=3*comptel_back**0.5/(comptel_aeff*comptel_exposure)#*comptel_energy**2
	...
	plot.annotate('ComPair', xy=(0.8,(3e-5*mev2erg)),xycoords='data',fontsize=22)

	#original amego AeffData
	AMEGO = numpy.loadtxt(fname='G_AllSourceSensitivities_0.csv', delimiter=',')
	plot.plot(AMEGO[:,0],AMEGO[:,1],color='red', alpha=1.0,lw=2)
	plot.annotate('AMEGO', xy=(0.8,(3e-7*mev2erg)),xycoords='data',fontsize=18,color='red')
